...
module.exports = {
  ...
  validate: function(validationInfo, props, required) {
    ...
    for(var newProp in props) {
      ...
      // gleiches gilt mit createdAt oder changedAt
      if (newProp === 'createdAt' || newProp === 'changedAt') {
        throw new errors.ValidationError(
          'Ungültiges Attribut ' + prop + '. `createdAt` und `changedAt` dürfen nie selbst gesetzt oder aktualisiert werden'
        );
      }
      // gleiches gilt mit status
      if (newProp === 'status') {
        throw new errors.ValidationError(
          'Ungültiges Attribut ' + prop + '. `status` darf nie selbst gesetzt oder aktualisiert werden'
        );
      }
